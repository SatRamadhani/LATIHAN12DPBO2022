...
class ProsesPasien implements KontrakPresenter
{
	...
	function cekUpdate()
	{
		try
		{
			if(isset($_POST['add']))
			{
				// $nik, $name, $place, $tl, $gender, $email, $telp
				$this->tabelpasien->addPasien($_POST['nik'], $_POST['name'],
					$_POST['place'], $_POST['date'], $_POST['gender'],
					$_POST['email'], $_POST['phone']);
			}
			else if(isset($_GET['id_hapus']))
			{
				$this->tabelpasien->deletePasien($_GET['id_hapus']);
		...
		{
			//memproses error
			echo "wiw error cekUpdate" . $e->getMessage();
		}
	}
